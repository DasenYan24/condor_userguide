...
共享存储可以理解为多台服务器共用的文件夹，用来存放代码、数据和运行环境，减少重复复制。

- `/mnt/cephfs`：由多台存储服务器共同提供的 CephFS 共享目录，新项目建议使用。
- `/mnt/net1`：由 `fengtianxiaoxu` 通过 NFS 提供的共享目录。

在已挂载相应目录且有权限的服务器上，相同路径访问的是同一份文件。`/mnt/cephfs` 和 `/mnt/net1` 是两套独立存储，内容不会自动同步。各节点的挂载情况见[共享存储映射](./servers/README.md#共享存储映射)。

共享存储和 HTCondor 文件传输可以同时使用：代码可传到执行节点的临时目录，环境和数据仍通过共享路径访问。`initialdir`、程序工作目录、结果回传位置的区别，以及三种方式的完整实践，见[共享存储、文件传输与作业目录](./storage/README.md)。

#### 文件应该放在哪里

| 路径 | 存储类型 | 适合存放 |
| --- | --- | --- |
| `/mnt/cephfs/<用户名>/` | CephFS HDD 容量区 | 项目代码、数据集、模型文件、计算结果等 |
| `/mnt/cephfs/hot-small/<用户名>/` | CephFS NVMe 区 | Python/Conda 环境、经常访问的小文件；避免堆放大型数据集 |
| `/mnt/net1/` | 原有 NFS 共享存储 | 已存放在这里的数据和运行环境 |

新项目建议在 `/mnt/cephfs/<用户名>/` 下组织文件，需要频繁访问的小文件和运行环境可放到 `hot-small` 下的个人目录。

强烈建议将 Conda 环境安装到 `/mnt/cephfs/hot-small/<用户名>/` 下面，速度快几倍。
...
- **[Python 程序](./Python/)** - 使用 `/mnt/cephfs/hot-small/dsyan/anaconda3` 中已有的环境，包含个人环境配置和作业提交示例
- **[存储与文件传输](./storage/README.md)** - 三个小型 CPU 示例，比较文件回传、共享数据和全部共享模式
